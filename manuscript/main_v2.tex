...
\caption{Trial--guideline concordance grid (HR+/HER2- mBC, v2 corpus). Each cell is the intersection of a patient state (x-axis) and biomarker profile (y-axis). Guideline decision nodes are shaded by their concordance status under strict tolerance: green = supported by $\ge 1$ trial edge satisfying all four concordance criteria, red = evidence-free, amber = mixed (multi-node cell with both supported and evidence-free nodes). G-IDs are labelled in the top-left of each guideline cell. Trial source nodes are shown as blue circles; the number inside each circle is the count of trial edges enrolling at that (state, biomarker) source.}
\label{fig:dag}
\end{figure}
...
Source code, schema, search query, LaTeX, and figures: \url{https://github.com/htlin222/mbc-evidence-dag-paper}, MIT licence. Zenodo DOI minted at \texttt{v2.0.0}.
