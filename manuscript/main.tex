...
All analysis was performed in Python 3.12 with \texttt{networkx} 3.5, \texttt{numpy} 1.26.4, \texttt{pandas} 2.2.3, \texttt{scipy} 1.17.1, and \texttt{matplotlib} 3.8.4; the full lockfile is in \texttt{uv.lock} at the tagged release commit. The source code, frozen schema, ESMO decision-tree encoding, and LaTeX sources are released at the GitHub URL in the Code Availability statement, and will be archived under a Zenodo DOI at the tagged release. The bootstrap random seed is \texttt{20260516}; two consecutive runs of \texttt{analysis/05\_compute\_efdpr.py} produce byte-identical \texttt{efdpr.json}.
...
\caption{HR+/HER2- mBC pivotal-trial DAG. Source nodes (blue circles) encode (prior-state, biomarker) tuples and are positioned by patient state (x-axis, ordered by treatment-line distance from treatment-naive) and biomarker profile (y-axis). Each labelled annotation records a trial that enrols at that source node, its primary-completion year, and the drug class it tests.}
\label{fig:dag}
\end{figure}
...
Source code, frozen JSON schema, LaTeX sources, and compiled PDF are available at \url{https://github.com/htlin222/mbc-evidence-dag-paper} under the MIT licence. The exact commit tagged \texttt{v1.0.0} reproduces all figures and tables. Persistent code DOI via Zenodo to be minted at release.
...
The author acknowledges the open-data infrastructure provided by ClinicalTrials.gov and the European Society for Medical Oncology guideline programme.
