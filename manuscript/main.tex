...
\textbf{Methods.}\, Sixteen pivotal phase 2/3 trials of HR+/HER2- mBC (and one PARP-inhibitor trial for the gBRCAm node), spanning primary-completion years 2014--2024, were retrieved from ClinicalTrials.gov via API v2 (freeze 2026-05-16). Eligibility criteria were structured into a frozen JSON schema (v1.0) by hand-curated extraction from canonical primary publications. The corpus was represented as a directed graph with nodes encoding (prior-state, biomarker) tuples and edges encoding pivotal trials. The ESMO mBC 2024 update was structured into a parallel 16-node decision tree. The evidence-free decision-point ratio (EFDPR) was the proportion of guideline nodes with no trial edge satisfying state-superset, biomarker, drug-class, and temporal-precedence criteria; three pre-registered tolerance levels (strict, ESCAT-aligned, liberal) were reported. The primary hypothesis was tested with the pre-registered one-sided exact binomial test of $H_0: \mathrm{EFDPR} \le 0.25$; Clopper-Pearson and bootstrap-percentile 95\% CIs accompanied each estimate.
\textbf{Results.}\, Under strict concordance the EFDPR was 0.31 (5/16 decision nodes; bootstrap 95\% CI 0.12--0.56; Clopper-Pearson 0.11--0.59); ESCAT-aligned gave the same value; liberal tolerance, accepting registrational biomarker-subgroup readouts, gave 0.25 (4/16; CI 0.07--0.52). The pre-registered one-sided exact binomial test of $H_0: \mathrm{EFDPR} \le 0.25$ failed to reject under all three tolerance levels (strict $P=0.37$; ESCAT $P=0.37$; liberal $P=0.60$). Evidence-free nodes under strict concordance clustered at post-CDK4/6i biomarker-stratified positions (ESR1mut, AKT-pathway, PIK3CAmut, no-actionable-mutation, and the post-CDK4/6i+post-endo everolimus node). Operationalization discordance was highest for the prior-CDK4/6i inclusion variable (ODI 0.91 across 7 trials) and for AKT-pathway alteration (ODI 0.67).
\textbf{Conclusion.}\, In this pre-registered 16-trial pilot the point estimate of the evidence-free decision-point ratio modestly exceeded the pre-registered 0.25 threshold under strict and ESCAT tolerance, but the pre-registered exact-binomial test did not reject the null at $\alpha=0.05$; the result is therefore best interpreted as descriptive evidence of substantial composition-across-non-overlapping-trials in post-CDK4/6i biomarker-stratified positions, with a definitive test requiring a systematic-review-scale corpus. The framework, schema, decision-tree encoding, and analysis code are released under MIT licence.
\end{abstract}
...
A graph-theoretic framework that places trial corpora and guideline decision trees on the same canonical (state, biomarker) node space and computes an evidence-free decision-point ratio (EFDPR). In a 16-trial / 16-node HR+/HER2- mBC pilot, the strict EFDPR point estimate was 0.31 (95\% CI 0.11--0.59); the pre-registered one-sided exact-binomial test of $H_0: \mathrm{EFDPR} \le 0.25$ did not reject ($P=0.37$). Evidence-free decision points clustered at post-CDK4/6i biomarker-stratified positions, and the prior-CDK4/6i inclusion definition was highly discordant across trials (ODI 0.91).
...
For each guideline node $g$, the supporting-evidence set $\mathrm{Supp}(g)$ is the set of trial-DAG edges $(u, v, t)$ that simultaneously satisfy: (a) \emph{state superset} --- every guideline-required prior-treatment token of $g$ is a member of the trial's source-state token set, with the special line-agnostic token \texttt{metastatic} (used for gBRCAm) satisfied by any state; (b) \emph{drug match} --- the trial drug class is identical to, or a family member of, one of the guideline-recommended classes at $g$; (c) \emph{biomarker concordance} under one of three pre-registered tolerance levels (strict: exact set match; ESCAT-aligned: strict plus ESCAT-equivalence groupings; liberal: ESCAT plus acceptance of registrational biomarker-subgroup readouts); (d) \emph{temporal precedence} --- the trial primary-completion year does not post-date the guideline-version year. The evidence-free decision-point ratio is
\[
...
Under strict concordance the EFDPR was 0.31 (5/16 decision nodes evidence-free; Clopper-Pearson 95\% CI 0.11--0.59; bootstrap percentile CI 0.12--0.56). Under ESCAT-aligned tolerance the EFDPR was identical at 0.31. Under liberal tolerance, accepting registrational biomarker-subgroup readouts, EFDPR was 0.25 (4/16; Clopper-Pearson 95\% CI 0.07--0.52). The point estimate exceeded the pre-registered 0.25 threshold under strict and ESCAT tolerance, but the pre-registered one-sided exact-binomial test of $H_0: \mathrm{EFDPR} \le 0.25$ did not reject under any tolerance level (strict $P = 0.37$; ESCAT $P = 0.37$; liberal $P = 0.60$; Table \ref{tab:efdpr}, Figure \ref{fig:efdpr}). The headline finding from this pilot is therefore a quantitative description of evidence sparsity at five clinically consequential decision points rather than a confirmatory rejection of the null. Evidence-free nodes under strict concordance were: post-CDK4/6i ESR1mut (G5), AKT-pathway (G6), PIK3CAmut (G7), no-actionable-mutation (G9), and the post-CDK4/6i+post-endo everolimus node (G13).
...
Two ESMO HR+/HER2- mBC decision nodes are supported by the guideline through citation of trials whose source-state encoding does not match the guideline's required state: the post-CDK4/6i PIK3CAmut node (G7), cited as supported by SOLAR-1 despite SOLAR-1's enrolment being post-AI rather than post-CDK4/6i; and the post-CDK4/6i AKT-pathway node (G6), cited as supported by CAPItello-291 despite CAPItello-291 enrolling a mixed population in which prior-CDK4/6i was permitted but not required. Under all three concordance tolerances, neither trial qualifies as a directly state-matched edge for the corresponding guideline node.

\subsection{Sensitivity analyses}
ESCAT-aligned tolerance left EFDPR unchanged at 0.31; the only ESCAT-equivalence in the HR+/HER2- mBC subset (AKT-pathway $\equiv$ PIK3CAmut) did not rescue the failed state-match for SOLAR-1 supporting G7. Liberal tolerance rescued EMERALD's support of the post-CDK4/6i ESR1mut node (G5), reducing EFDPR to 0.25, exactly at the pre-registered threshold. ASCO and NCCN sensitivity analyses are deferred to the production-scale extension.
